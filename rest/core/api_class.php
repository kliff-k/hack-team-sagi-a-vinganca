    ...
    private function execSource ()
    {
        ...
        switch ($this->path[1])
        {
            case 'banner':
                ...
                {
                    case 'GET':
                        $return = json_decode(file_get_contents("https://hs2019st.com/govbr/solr-select?q=*:*&views_i%20desc&rows=15"),TRUE);
                        break;
                }
                break;
            case 'cards':
                switch ($this->method)
                {
                    case 'GET':
                        $area = $this->path[2];
                        $return = json_decode(file_get_contents("https://hs2019st.com/govbr/solr-select?q=*:*&views_i%20desc&fq=areas_de_interesse_ss:'$area'"),TRUE);
                        break;
                }
                break;
        }
